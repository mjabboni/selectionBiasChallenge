...
This project uses the following Python modules to create the selection bias visualization:

### Core Pipeline Modules

1. **step1_prepare_image.py** - Prepares and normalizes input images
2. **step2_create_stipple.py** - Creates blue noise stippling patterns
3. **step3_create_tonal.py** - [Optional] Performs tonal analysis for parameter tuning
4. **step4_create_block_letter.py** - Generates block letter masks for selection bias
5. **step5_create_masked.py** - Applies masks to create biased estimates
6. **create_meme.py** - Assembles the final four-panel meme visualization

### Supporting Modules

7. **importance_map.py** - Computes importance weights for stippling (identifies which regions should receive more dots)
8. **stippling_functions.py** - Implements void-and-cluster blue noise algorithm with toroidal Gaussian kernels
